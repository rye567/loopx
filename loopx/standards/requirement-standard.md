# 需求标准

## 目的

在方案、代码或测试开始前，让每个用户请求都变成可执行需求。

## 必需输入

- 用户请求或 issue 描述。
- 目标项目和模块，如果已知。
- 已知业务目标、用户角色或流程。
- 用户已声明的约束、期限、依赖或非目标。

## 必需输出

需求产物必须包含：

| 字段 | 要求 |
| --- | --- |
| `id` | 稳定需求 id，例如 `REQ-20260515-001`。 |
| `title` | 简短、动作导向的标题。 |
| `background` | 为什么需要这个变更。 |
| `users` | 谁使用或受影响。 |
| `problem` | 当前痛点、缺陷或缺失能力。 |
| `goals` | 期望的可观察结果。 |
| `in_scope` | 将会变更什么。 |
| `out_of_scope` | 不会变更什么。 |
| `acceptance_criteria` | 写成可观察行为的可验证标准。 |
| `edge_cases` | 边界、空值、非法输入、并发或权限场景。 |
| `non_functional_requirements` | 简单性、安全、性能、扩展、兼容、可观测性和可靠性需求；不适用项说明理由。 |
| `risks` | 需求歧义、数据、安全、集成和发布风险。 |
| `dependencies` | 需要的系统、团队、配置、数据或工具。 |
| `open_questions` | 进入下一检查前必须澄清的问题。 |

## 通过标准

需求检查只有在以下条件满足时才能通过：

- 验收标准无需解释意图即可测试。
- 范围和非范围都明确。
- 受影响用户或流程已命名。
- 边界场景和失败路径已列出，或明确标记为不适用。
- 性能要求如适用，给出目标来源和可复现的负载、环境或数据条件，不使用 LoopX 通用数值。
- 身份、权限、租户、输入、敏感数据、依赖和外部副作用风险已按实际范围标注。
- 开放问题为空、非阻塞，或分配给 `NEED_HUMAN`。
- 有足够风险标签用于选择 `LIGHT`、`STANDARD` 或 `FULL`。

## 失败 / 返回规则

- 用户目标缺失：返回需求访谈。
- 验收标准缺失：返回验收标准草拟。
- 范围模糊：返回范围边界分析。
- 安全、权限、数据迁移或外部副作用不确定：阻塞或升级为 `FULL`。

## 证据

智能体必须把需求产物路径、未解决问题、已选风险标签、非功能需求来源和检查结果写入阶段结果。
